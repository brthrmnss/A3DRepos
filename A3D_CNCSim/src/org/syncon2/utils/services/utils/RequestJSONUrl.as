package org.syncon2.utils.services.utils
{
	...
	public class RequestJSONUrl
	{
		...
		public function postUrl( url : String , params : Object=null ,  returnMessagesFx : Function=null  ): AsyncToken
		{			
			url = cleanUrl( url ) ; 
		...
		private function onRecievedStream(pEvt:ResultEvent):void
		{
			...
			var parseJSON : Boolean = true
			if ( token.hasOwnProperty( 'parseJSON' ) && token.parseJSON == false ) 
				parseJSON = false 
		...
		public function onFault(pFaultEvt:  FaultEvent):void
		{
			...
			trace('RequestJSONUrl', 'onFault', 'failure', fault.faultString, fault.faultDetail ) 
